...
**工具栏左侧状态条（BusyChip）**：显示"正在同步……"与"正在保存……"。位于 `TopAppBar` 的 `navigationIcon` 槽位，因而与右上角设置按钮同高，且工具栏高度恒定，出现/消失不会推动下方内容。

同步期间**同时**显示左侧文字与正中间圆圈，这是明确要求：文字说明"在做什么"，圆圈说明"还在进行"。不要为了"去重"而删掉其中一个。

**同步指示器**：`SyncSpinner` 由 `topBar` 外的 `Box` 用 `Alignment.Center` 定位在工具栏正中间，`PullToRefreshBox` 的 `indicator` 必须是空 lambda `{}`。

它同时承担两种状态：手指按住时显示拖拽进度（determinate arc），同步进行时旋转。**一个组件覆盖两种情况是刻意的**——分开做就会产生两个圆圈。

两个已经踩过、不要再犯的坑：

1. **不要放进 `TopAppBar` 的 title 槽位。** title 是在 navigationIcon 与 actions 之间布局的，**它的中点不是整条栏的中点**：实测 x≈482，而屏幕中心是 540。必须叠在工具栏之上居中。当前实测（同步中，四处同在 y=188）：

   | 元素 | x 范围 | 中心 |
   |---|---|---|
   | 左侧小圆圈 | 77..116 | (96, 188) |
   | 左侧文字 | 138..336 | (237, 188) |
   | 正中间圆圈 | 513..568 | **(540, 188)** |
   | 设置按钮 | 942..1008 | (975, 188) |
