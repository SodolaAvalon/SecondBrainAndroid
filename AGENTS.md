...
**内容区不允许出现任何圆形刷新/同步指示器**：`PullToRefreshBox` 的 `indicator` 必须保持为空 lambda `{}`。

原因：`isRefreshing` 绑定的是 `sync.isSyncing`，而它**对用户未发起的后台同步同样为真**。只要内容区画了指示器，就必然与工具栏状态条重复。

已经踩过两次：第一次写 `visible = isRefreshing || drag > 0f`，圆圈常显；第二次改成只看 `drag > 0f`，圆圈在**下拉过程中**依然出现。所以不要再去调可见性条件——**不画**才是唯一稳定的解法。

接受的代价：下拉未达阈值时不再有屏幕反馈；同步状态一律由工具栏状态条承担。下拉刷新功能本身不受影响。
