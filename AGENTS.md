...
**工具栏左侧状态条（BusyChip）**：只用于**保存**（"正在保存……"）。位于 `TopAppBar` 的 `navigationIcon` 槽位，因而与右上角设置按钮同高，且工具栏高度恒定，出现/消失不会推动下方内容。**同步不要用它**——同步由正中间的圆圈表示，两者同时出现即重复状态。

**同步指示器只有一个，位于工具栏正中间**：`SyncSpinner` 由 `topBar` 外的 `Box` 用 `Alignment.Center` 定位，`PullToRefreshBox` 的 `indicator` 必须是空 lambda `{}`。

它同时承担两种状态：手指按住时显示拖拽进度（determinate arc），同步进行时旋转。**一个组件覆盖两种情况是刻意的**——分开做就会产生重复圆圈。

两个已经踩过、不要再犯的坑：

1. **不要放进 `TopAppBar` 的 title 槽位。** title 是在 navigationIcon 与 actions 之间布局的，**它的中点不是整条栏的中点**：实测 x≈482，而屏幕中心是 540。必须叠在工具栏之上居中，现在实测正好在 x=540、y=188（与设置按钮同一水平线）。
2. **`indicator` 不要用 Material 默认实现。** 默认指示器挂在**内容区**顶部中间，而且 `isRefreshing` 为真时会一直显示——包括用户未发起的后台同步，于是又变成内容区里的野生圆圈。这正是它被移除两次的原因。

分工：同步 = 正中间圆圈（左侧 BusyChip **不显示**，否则文字与圆圈同时出现又成了重复状态）；保存 = 左侧 BusyChip 文字（保存没有圆圈，所以文字是唯一指示）。

页码不跳动：`SyncSpinner` 的 `AnimatedVisibility` 只做淡入与轻微缩放，不参与布局测量，工具栏高度恒定。
